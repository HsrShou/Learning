...
<div>
    <form action="goods" method="get">
        <input type="hidden" name="url" value="goodsListPageCondition">
        ...
        <select name="gtId">
            ...
            <c:forEach items="${gtList}" var="goodsType">
                <option name="gtId" ${goods.gtId == goodsType.typeId? "selected" : "" }
                        value="${goodsType.typeId}">${goodsType.typeName} </option>
            </c:forEach>
        </select>
...
<table id="tb1">
    <c:forEach items="${page.data}" var="goods" varStatus="s">
        ...
        <td>
            <div style="width:200px;height:200px;cursor:pointer">
                <table>
                    <tr>
                        <td>
                            <a href=""><img width="128px" src="${goods.imgUrl}"></a>
                        </td>
                    </tr>
                    <tr>
                        <td>名称：${goods.goodsName}</td>
                    </tr>
                    <tr>
                        <td>价格：${goods.unitPrice}</td>
                    </tr>
                    <tr>
                        <td><a href=""
...
<script>
    ...
    function test1(currentPage) {
        //去后台
        window.location.href = 'goods?url=goodsListPage&type=1&currentPage=' + currentPage;
    }
</script>
<%--${goodsList}--%>
</body>
